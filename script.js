class TerminalResume {
  ...
  async init() {
    ...
    if (this.portfolioData) {
      ...
      document.title = `${name} | Terminal Resume`;
      
      const cleanName = name.replace(/\s+/g, '').toLowerCase();
      const titleEl = document.querySelector('.terminal-title');
      if (titleEl) {
        titleEl.textContent = `${cleanName}: ~/resume`;
      }
    }
...
╚═╝     ╚═╝╚═╝  ╚═╝╚═╝  ╚═╝╚═╝ ╚═════╝ `;
  ...
  showAbout(outputElement = this.output) {
    const personal = this.portfolioData ? this.portfolioData.personal : { name: "Marjo Ballabani", role: "Senior Software Engineer", bio: "Senior software engineer with more than 10 years of programming experience." };
    const bioLines = this.chunkText(personal.bio, 55);
    const boxContent = bioLines.map(line => {
      const padded = line.padEnd(55, ' ');
      return `${this.wrapWithColor("│", "#ff8c00")} ${this.wrapWithColor(padded, "#ffffff")} ${this.wrapWithColor("│", "#ff8c00")}`;
    }).join('\n');
...
${this.wrapWithColor(
  `   Building scalable and efficient software solutions as a`,
  "#ffffff"
)}
${this.wrapWithColor(`   ${personal.role}`, "#ff8c00")}

${this.wrapWithColor("⚡ Location", "#ff8c00")}
${this.wrapWithColor(
...
${this.wrapWithColor("│", "#ff8c00")} ${this.wrapWithColor(
      "Ready to bring your innovative ideas to life!".padEnd(51),
      "#ffffff"
...
${this.wrapWithColor(
  ...
  showExperience(outputElement = this.output) {
    let experience = `<span style="color: #ffff00; font-weight: bold;">💼 Professional Experience</span>\n\n`;

    const journey = this.portfolioData ? this.portfolioData.journey : [
      {
        company: "Unicepta",
        role: "Senior Software Engineer",
        period: "Jul 2020 - Nov 2025",
        location: "Cologne, Germany",
        description: "Part of Core team working on AI-powered Media & Data Intelligence Solutions."
      }
    ];

    journey.forEach(job => {
      experience += `<span style="color: #00ffff;">${job.company.toUpperCase()} | ${job.role}</span>\n`;
      experience += `${this.wrapWithColor(job.period + " | " + job.location, "#ffffff")}\n\n`;

      const bullets = job.description.split('. ').filter(Boolean);
      bullets.forEach(b => {
        let bText = b.trim();
        if (bText && !bText.endsWith('.')) bText += '.';
        experience += `• ${this.wrapWithColor(bText, "#ffffff")}\n`;
      });
      experience += `\n`;
    });
  ...
  showEducation(outputElement = this.output) {
    const edu = this.portfolioData && this.portfolioData.education ? this.portfolioData.education[0] : {
    ...
    const degreeLine = edu.degree.padEnd(46, ' ');
...
${this.wrapWithColor("📍 Location:", "#ff8c00")}    ${this.wrapWithColor(
  ...
  showSkills(outputElement = this.output) {
    let skills = `<span style="color: #ffff00; font-weight: bold;">🛠️ TECHNICAL SKILLS</span>\n\n`;

    if (this.portfolioData && this.portfolioData.skills) {
      Object.entries(this.portfolioData.skills).forEach(([cat, info]) => {
        skills += `• ${this.wrapWithColor(cat + ":", "#ff8c00")} ${this.wrapWithColor(info.tags.join(", "), "#ffffff")}\n`;
      });
    } else {
      skills += `• ${this.wrapWithColor("Programming:", "#ff8c00")} TypeScript, Python, JavaScript, Java\n`;
      skills += `• ${this.wrapWithColor("Frontend:", "#ff8c00")} React, Angular\n`;
      skills += `• ${this.wrapWithColor("Cloud & DevOps:", "#ff8c00")} GCP, AWS, Docker, Kubernetes\n`;
    }
  ...
  showContact(outputElement = this.output) {
    const personal = this.portfolioData ? this.portfolioData.personal : {
      email: "marjoballabani@gmail.com",
      social: {
        github: "https://github.com/marjoballabani",
        linkedin: "https://linkedin.com/in/marjoballabani"
      }
    };
...
${this.wrapWithColor("┌────────────────────────────────────────┐", "#ff8c00")}
${this.wrapWithColor("│", "#ff8c00")} ${this.wrapWithColor(
      "Let's connect and create something great!".padEnd(38),
      "#ffffff"
    )} ${this.wrapWithColor("│", "#ff8c00")}
${this.wrapWithColor("└────────────────────────────────────────┘", "#ff8c00")}
...
${this.wrapWithColor("✉", "#ff8c00")}  ${this.wrapWithColor(
      ...
      `<a href="mailto:${personal.email}" style="color: #ffffff; text-decoration: none;">${personal.email}</a>`,
      "#ffffff"
    )}
...
${this.wrapWithColor("⚡", "#ff8c00")}  ${this.wrapWithColor(
      ...
      `<a href="${personal.social.github}" target="_blank" style="color: #ffffff; text-decoration: none;">${personal.social.github}</a>`,
      "#ffffff"
    )}
...
${this.wrapWithColor("💼", "#ff8c00")}  ${this.wrapWithColor(
      ...
      `<a href="${personal.social.linkedin}" target="_blank" style="color: #ffffff; text-decoration: none;">${personal.social.linkedin}</a>`,
      "#ffffff"
    )}`;
  ...
  loadProjects() {
    if (this.portfolioData && this.portfolioData.projects) {
      this.projects = this.portfolioData.projects.map(p => ({
        title: p.name,
        description: p.tagline,
        image: p.logoImg || "image/avatar-gpt.png",
        technologies: [],
        demo: p.demoUrl,
        repo: p.repoUrl
      }));
  ...
  loadSkills() {
    if (this.portfolioData && this.portfolioData.skills) {
      this.skills = {};
      Object.entries(this.portfolioData.skills).forEach(([cat, info]) => {
        this.skills[cat.toLowerCase()] = {};
        info.tags.forEach(t => {
          this.skills[cat.toLowerCase()][t] = Math.floor(Math.random() * 15) + 80;
        });
      });
    } else {
      this.skills = {
  ...
  generateLinkedInCover(outputElement) {
    const name = this.portfolioData ? this.portfolioData.personal.name : "Marjo Ballabani";
    const cleanName = name.replace(/\s+/g, '').toLowerCase();
    ...
    terminalTitle.textContent = `${cleanName}: ~/interactive-resume`;
